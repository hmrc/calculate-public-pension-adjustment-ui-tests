...
import uk.gov.hmrc.test.ui.functions.{CommonCalculationAAandSchemePaidLTA, CommonCalculationAAandUserPaidLTA}
...
class EndToEndAAAndLTAJourney2Test extends BaseSpec with BeforeAndAfter {
  ...
  before {
    ...
    val commonCalculationAAAndSchemePaidLTA      = new CommonCalculationAAandSchemePaidLTA()
    ...
    val uniqueValuesSet                          = scala.collection.mutable.HashSet[String]()
    val keysToRemove                             = scala.collection.mutable.ListBuffer[String]()

    for ((key, value) <- taxSchemes)
      if (uniqueValuesSet.contains(value)) {
        // If the value is not unique, mark the key for removal
        keysToRemove += key
      } else {
        // If the value is unique, add it to the set of unique values
        uniqueValuesSet += value
      }

    // Remove the marked keys from the map
    keysToRemove.foreach(taxSchemes -= _)
  }
